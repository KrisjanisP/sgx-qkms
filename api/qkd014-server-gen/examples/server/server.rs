//! Main library entry point for qkd014_server_gen implementation.
...
pub async fn create(addr: &str, https: bool) {
    ...
    #[allow(unused_mut)]
    let mut service =
        qkd014_server_gen::server::context::MakeAddContext::<_, EmptyContext>::new(
            service
        );
...
use qkd014_server_gen::server::MakeService;
use std::error::Error;
...
impl<C> Api<C> for Server<C> where C: Has<XSpanIdString> + Send + Sync
{
    ...
    {
        info!("get_key(\"{}\", {:?}) - X-Span-ID: {:?}", slave_sae_id, key_request, context.get().0.clone());
    ...
    {
        info!("get_key_with_ids(\"{}\", {:?}) - X-Span-ID: {:?}", master_sae_id, key_ids, context.get().0.clone());
        Err(ApiError("Api-Error: Operation is NOT implemented".into()))
    }
    ...
    {
        info!("get_key_with_ids_simple(\"{}\", {:?}) - X-Span-ID: {:?}", master_sae_id, key_id, context.get().0.clone());
        Err(ApiError("Api-Error: Operation is NOT implemented".into()))
    }
    ...
    {
        info!("get_status(\"{}\") - X-Span-ID: {:?}", slave_sae_id, context.get().0.clone());
        Err(ApiError("Api-Error: Operation is NOT implemented".into()))
    }
